...
// популярные товары
$db = getDB();
$stmt = $db->prepare('SELECT p.*, c.name AS category_name, COALESCE((SELECT SUM(ps.quantity) FROM product_sizes ps WHERE ps.product_id = p.id), 0) AS total_stock FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.is_featured = 1 AND p.is_active = 1 ORDER BY p.created_at DESC LIMIT 8');
$stmt->execute();
